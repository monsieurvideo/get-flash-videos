...
Download the Flash video from the web page given in C<URL>, choosing a
suitable filename. Use C<--filename> if you want to set the filename
yourself. If C<search> is specified (either quoted or unquoted),
C<get_flash_videos> will search Google Video for C<search>, and present a
list of results.

I<get-flash-videos> attempts to support many video sites, therefore there is no
...
Save downloaded via to the specified filename. Usually this shouldn't be
...
Print the version of I<get_flash_videos>.
...
On startup I<get_flash_videos> will read the configuration files located at
...
If you'd like to change or improve I<get-flash-videos> (for example adding
