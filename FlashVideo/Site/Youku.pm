...
sub find_video {
	...
	my $check_response = sub {
		...
		error sprintf $message, $browser->response->code;
		exit 1;
	};
	...
	if ( $embed_url !~ m`^http://v.youku.com/v_show/` )
	{
		...
		error "Don't recognise the youku link" and exit 1
	...
	error "Couldn't extract video ID from youku page, we are probably out of date" and exit 1
		unless $videoID;
...
=cut
	...
	error "Can't find the seed value in the video info JSON" and exit 1
		unless $shuffle_seed;
	...
	error "Youku rejected our attempt to get the video, we're probably out of date" and exit 1
